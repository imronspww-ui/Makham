...
export default function PosPage() {
  ...
  function addToCart(item: MenuItem) {
    setCart((prev) => {
      ...
      return [...prev, { menuItemId: item.id, name: item.name, price: item.price, qty: 1, imageUrl: item.imageUrl }]
    })
  }

  function setQty(menuItemId: string, qty: number) {
    if (qty <= 0) return removeFromCart(menuItemId)
    setCart((prev) => prev.map((i) => i.menuItemId === menuItemId ? { ...i, qty } : i))
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      await createOrder({
        ...
        total,
        note:         discountAmount > 0 ? `ส่วนลด ${discountType === 'percent' ? discountInput + '%' : formatCurrency(discountAmount)}` : '',
        status:       'completed',
      })
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.menuItemId}
                ...
                <p className="text-xs text-orange-500 font-semibold shrink-0">{formatCurrency(item.price)}</p>
                {/* Qty */}
                ...
                {/* Subtotal */}
                <p className="text-sm font-bold text-gray-700 w-14 text-right shrink-0">{formatCurrency(item.price * item.qty)}</p>
